...
const Falencia = () => {
  ...
  const caseType = (content.caseTypes || []).find(c => c && c.id === "falencia");


  const filteredCompanies = (content.companies || []).filter(company => {

    const matchesCaseType = company.caseType === "falencia";
  ...
  const comarcas = Array.from(new Set((content.companies || []).filter(c => c && c.caseType === "falencia").map(c => c.comarca)));
  const ufs = Array.from(new Set((content.companies || []).filter(c => c && c.caseType === "falencia").map(c => c.uf)));
