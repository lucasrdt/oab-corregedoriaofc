...
const Litisconsorcio = () => {
  ...
  const caseType = (content.caseTypes || []).find(c => c && c.id === "litisconsorcio");


  const filteredCompanies = (content.companies || []).filter(company => {

    const matchesCaseType = company.caseType === "litisconsorcio";
  ...
  const comarcas = Array.from(new Set((content.companies || []).filter(c => c && c.caseType === "litisconsorcio").map(c => c.comarca)));
  const ufs = Array.from(new Set((content.companies || []).filter(c => c && c.caseType === "litisconsorcio").map(c => c.uf)));
