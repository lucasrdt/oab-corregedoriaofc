...
import { Loader2, Plus, UserX, ExternalLink, FileText, RotateCcw, UserCircle, Users, Building2, FolderOpen, Globe, Trash2 } from 'lucide-react';
...
const UsuariosSection = () => {
  ...
  const handleResetPassword = async (userId: string) => {
    ...
    toast.success('Senha resetada para Mudar@123. O usuário deverá trocá-la no próximo login.');
  };
  ...
  const executeDeleteUser = async (roleId: string) => {
    const { error } = await supabase
      .from('user_roles')
      .delete()
      .eq('id', roleId);
          ...
          <DialogContent className="sm:max-w-md border-none shadow-2xl overflow-hidden p-0">
            <DialogHeader className="bg-primary p-6 text-white border-b border-primary/10">
              <DialogTitle className="text-sm font-black uppercase tracking-widest flex items-center gap-2">
                <Plus className="h-4 w-4 text-[#C1A461]" /> Novo Colaborador
              </DialogTitle>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                A senha padrão <span className="font-mono font-medium">Mudar@123</span> será definida automaticamente. O usuário deverá trocá-la no primeiro login.
              </p>
                ...
                users.map((u) => (
                  <TableRow key={u.id} className="hover:bg-muted/30 transition-colors border-border/30">
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`${u.role === 'admin' ? 'bg-red-500/10 text-red-600 border-red-500/20 hover:bg-red-600 hover:text-white' : u.role === 'dev' ? 'bg-secondary/10 text-secondary border-secondary/20 hover:bg-secondary hover:text-white' : 'bg-primary/10 text-primary border-primary/20 hover:bg-primary hover:text-white'} text-[9px] font-black uppercase tracking-widest px-2 py-0.5 border transition-all duration-200 cursor-default`}
                      >
                        {u.role}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`${u.active ? 'bg-green-500/10 text-green-600 border-green-500/20 hover:bg-green-600 hover:text-white' : 'bg-muted text-muted-foreground border-border hover:bg-muted-foreground hover:text-white'} text-[9px] font-black uppercase tracking-widest px-2 py-0.5 border transition-all duration-200 cursor-default`}
                      >
      ...
      <Dialog open={!!userToDelete} onOpenChange={(open) => !open && setUserToDelete(null)}>
        <DialogContent className="sm:max-w-md border-none shadow-2xl overflow-hidden p-0 bg-white">
          <DialogHeader className="bg-red-600 p-6 text-white border-b border-red-700">
            <DialogTitle className="text-sm font-black uppercase tracking-widest flex items-center gap-2">
              <Trash2 className="h-4 w-4 text-white" /> Confirmar Exclusão
            </DialogTitle>
            <span className="sr-only">Confirme a exclusão permanente do usuário</span>
          </DialogHeader>
          <div className="p-6 space-y-4">
            <p className="text-sm text-slate-600 leading-relaxed font-medium">
              Tem certeza de que deseja excluir **permanentemente** este colaborador e todas as suas permissões de acesso?
            </p>
            <div className="text-xs text-red-500 font-bold bg-red-50 border border-red-100 p-3 rounded-lg flex items-start gap-2">
              <span className="text-sm mt-0.5">⚠️</span>
              <span>Esta ação é irreversível e removerá a conta do usuário também da base de autenticação do Supabase.</span>
            </div>
          ...
          <Table>
            <TableHeader className="bg-primary/5">
              <TableRow className="hover:bg-transparent border-border/50">
                ...
                <TableHead className="text-primary font-black text-[10px] uppercase tracking-widest py-5">Nº Processo</TableHead>
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) setCoverImageUrl(''); }}>
          ...
          <DialogContent className="sm:max-w-lg border-none shadow-2xl overflow-hidden p-0">
            <DialogHeader className="bg-primary p-6 text-white border-b border-primary/10">
              <DialogTitle className="text-sm font-black uppercase tracking-widest flex items-center gap-2">
                <Plus className="h-4 w-4 text-[#C1A461]" /> Nova Subseção
              </DialogTitle>
