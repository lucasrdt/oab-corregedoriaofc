...
const RecuperacaoJudicial = () => {
  ...
  const caseType = (content.caseTypes || []).find(c => c && c.id === "recuperacao-judicial");


  // Filtrar empresas apenas de recuperação judicial
  const filteredCompanies = (content.companies || []).filter(company => {

    const matchesCaseType = company.caseType === "recuperacao-judicial";
  ...
  const comarcas = Array.from(new Set((content.companies || []).filter(c => c && c.caseType === "recuperacao-judicial").map(c => c.comarca)));
  const ufs = Array.from(new Set((content.companies || []).filter(c => c && c.caseType === "recuperacao-judicial").map(c => c.uf)));
