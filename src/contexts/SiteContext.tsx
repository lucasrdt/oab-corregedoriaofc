...
export const SiteProvider = ({ children }: { children: ReactNode }) => {
    ...
    const fetchConfig = useCallback(async () => {
        ...
        try {
            ...
            console.error('[SiteContext] Erro ao buscar configuração (usando template local como fallback):', err);
            // Sempre usa o template estático como fallback — o site nunca fica bloqueado
            setConfig(fullIvaldoTemplate);
            setSiteId(null);
            setError(null); // não exibe tela de erro; o fallback já cobre
        } finally {
            setLoading(false);
        }
    }, []);
