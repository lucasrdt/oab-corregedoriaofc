...
const MostConsultedProcesses = () => {
  ...
  const { data: processes = [], isLoading } = useQuery({
    queryKey: ["highlighted-processes"],
    queryFn: async () => {
  ...
  const mappedProcesses = processes.map((p: any) => ({
    ...p,
    name: p.nome,
    initials: p.initials || getInitials(p.nome),
    linkHabilitacoes: p.link_habilitacoes,
    especialistaResponsavel: p.especialista,
  }));

  if (isLoading) {
    return (
      <section ref={ref} className="scroll-fade-in visible py-12">
        <div className="container mx-auto flex flex-col items-center justify-center min-h-[200px]">
      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {mappedProcesses.map((process) => (
            <a
              key={process.id}
              href={`/${getCompanySlug(process, mappedProcesses)}`}
