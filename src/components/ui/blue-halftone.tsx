...
// ShaderBackground — base "Halftone" do 21st.dev Shader Builder,
// adaptado para a paleta clara da Corregedoria OAB-MA.
// Zero dependências: um canvas WebGL que preenche o pai.
// Uso: <div className="relative"><ShaderBackground className="absolute inset-0" />…
//
// AJUSTES vs. original:
//  - Paleta trocada para azul-marinho/slate institucional (claro)
//  - brightness invertido para positivo (base clara em vez de escura)
//  - vignette zerada (não sujar os cantos sobre fundo branco)
//  - saturation reduzida (tom discreto, institucional)
//  - acento vermelho OAB reservado como última cor da rampa
//  - guard de prefers-reduced-motion (fallback estático)
...
// ─────────────────────────────────────────────────────────────────────────────
// PALETA E AJUSTES — mexa só aqui.
//
// Cores em RGB normalizado (0–1). Conversão: canal / 255.
//   u_colors[0] é a "base" (fundo entre os pontos) — mantida quase branca
//   para o texto escuro da hero permanecer legível por cima.
//   As demais formam a rampa dos pontos: slate → azul-marinho OAB,
//   com o vermelho OAB (#C8102E) como acento final.
// ─────────────────────────────────────────────────────────────────────────────
const UNIFORMS = {
  colors: [
  ...
  [0.118, 0.251, 0.686], // #1E40AF
  [0.118, 0.227, 0.541], // #1E3A8A
  [0.118, 0.227, 0.541],
  [0.118, 0.227, 0.541],
  [0.118, 0.227, 0.541],
...
colorCount: 5,
  ...
  detail: 4.000,
  contrast: 0.600,   
  brightness: -0.400, //negativo = base escura (tom azul original)
  saturation: 0.700,   hue: 0.0000,
  vignette: 1.000,   
  blur: 0.0400,
  grain: 0.090,      
  seed: 4990.0,
  ...
  offsetX: -0.250,
  offsetY: -0.020,
  drift: 0.100,        cursorEnabled: true,
  cursorEffect: 2.0, // swirl no cursor
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // ── Guard de acessibilidade: prefers-reduced-motion ──────────────────────
    // Se o usuário pediu menos movimento, não iniciamos o loop WebGL.
    // Renderizamos um fallback estático (um frame único) e paramos.
    ...
    // ── Caminho reduced-motion: desenha UM frame estático e encerra ──────────
    if (reduceMotion) {
      const { width, height } = resizeCanvas()
      gl.uniform4f(uni.scene, width, height, 0, UNIFORMS.colorCount)
