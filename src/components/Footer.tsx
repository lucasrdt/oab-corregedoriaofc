  ...
  return (
    <footer className="bg-navy-deep text-white border-t border-white/5">
      ...
      <div className="bg-black/10 border-b border-white/5">
        <div className="container-padding py-6 flex items-center justify-between transition-all duration-500">
           <span className="text-[10px] font-bold tracking-[0.3em] uppercase text-white">Institucional</span>
           <div className="flex items-center gap-8">
              {(content.footer?.partners?.length ? content.footer.partners : ["OAB Nacional", "CNJ", "Justiça"]).map((partner) => (
                <div key={partner} className="h-6 w-24 bg-white/10 rounded flex items-center justify-center text-[9px] font-bold text-white uppercase tracking-tighter">
                  {partner}
                </div>
              ))}
           </div>
